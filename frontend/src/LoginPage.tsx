...
import { Gavel, User, Lock, ArrowRight, Eye, EyeOff, Brain, TrendingUp, ShieldCheck, Calendar } from 'lucide-react';
...
const S = {
  ...
  footer: {
    textAlign: 'center' as const, fontSize: '1rem', color: '#c2c6d6',
  },
};
...
export default function LoginPage() {
  ...
  return (
    <div style={S.page}>
      ...
      {/* Left column — hidden on mobile via className, width 50% on desktop */}
      ...
      <main className="w-full lg:w-1/2" style={S.rightCol}>
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.4 }} style={S.card}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
            ...
            <div style={{ textAlign: 'center' }}>
              <h2 style={S.cardHeading}>NyayaDrishti</h2>
              <p style={S.cardSubheading}>Welcome Back. Please log in to continue.</p>
            </div>
          </div>
          ...
          <AnimatePresence>
            {error && (
              <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                style={{ marginBottom: '0.5rem', padding: '0.75rem', borderRadius: '8px', backgroundColor: 'rgba(255,180,171,0.1)', border: '1px solid rgba(255,180,171,0.2)', color: '#ffb4ab', fontSize: '12px', fontWeight: 600 }}>
                {error}
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              <label style={S.fieldLabel} htmlFor="userId">User ID</label>
              <div style={S.inputWrap}>
                <div style={S.iconWrap}><User style={{ width: '18px', height: '18px' }} /></div>
                <input id="userId" type="text" placeholder="Enter your User ID" value={userId} onChange={e => setUserId(e.target.value)} required
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
              ...
              <div style={S.inputWrap}>
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)}
                  style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#8c909f', padding: 0 }}>
            ...
            <div style={S.checkRow}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
                ...
                <span style={{ fontSize: '0.875rem', fontWeight: 500, color: '#c2c6d6' }}>Remember me</span>
              </label>
              <a href="#" style={{ fontSize: '0.875rem', fontWeight: 500, color: '#3b82f6', textDecoration: 'none' }}>
                Forgot Password?
              </a>
            </div>
            ...
            <div style={{ marginTop: '0.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button type="submit" disabled={isLoading}
                ...
                {isLoading ? 'Processing...' : (<>LOGIN <ArrowRight size={18} /></>)}
              </button>
